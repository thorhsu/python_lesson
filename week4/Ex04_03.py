...
found = True
while found:
    startPosition = text.find(find_str, startPosition)
    found = (startPosition >= 0)
    if found:
        findCount += 1
        print("第%d個%s在%d位置" %(findCount, find_str, startPosition))
        startPosition += len(find_str)
   
print("總共有%(findCount)d個“%(find_str)s”" %{"findCount": findCount, "find_str": find_str})
